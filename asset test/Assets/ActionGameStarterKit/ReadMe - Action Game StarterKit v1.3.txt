
----------------------------------------------------------------------
			Unity3D Action Game StarterKit Ver 1.3
----------------------------------------------------------------------	
		
I hope this package will help you to develop basic action game using Unity3D.
This package has been saved Unity3D ver 4.2.2f1 and also tested Unity3D ver 4.3.1f1.

This package has everything you need to create a basic top-down fighting game, complete with controls, animations, enemies, fighting, pickups, levelling-up and spawn system. 

Get a head start with this simple, clean and elegant starter kit!

Action Game Starter Kit consists of
- Character parts system.
- Player's basic & range attack.
- Summon creature
- Auto targeting system.
- Boss & Monster AI.
- Pet system.
- Item pickup for weapon change.
- Boss & Monster spawn system.
- Level up system.
- PC & Mobile is supported.
- And more.

if you have any questions or suggestions, just mail me.
My Email is insaneoops@naver.com

--------------------------------
         Version History
--------------------------------
1.3.0
- Character Parts Change System is added.
- Minimap is added.
- Summon Creature is added.	
1.13
- Select Character Menu is added.
- Joystick script is changed.
1.12
- Range Attack is added.
1.11
Bug fix.
1.1
- Joystick controller is added for mobile device now.
1.01
- English comments is added	
1.0
- Simple third-person camera action game using sword and gun.
- Simple pet and weapon change system.
- 8 direction movement and auto targeting.
- All scripts was saved C# language, you can easy to understand the scripts, I hope.