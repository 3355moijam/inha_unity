...
[AddComponentMenu("Insaneoops/csMonsterDeathBody")]
public class csMonsterDeathBody : MonoBehaviour 
{	
	private void Start () 
	{	
		csSetting mySetting = (csSetting)FindObjectOfType (typeof (csSetting));
		GetComponent<Animation>().AddClip (mySetting.monsterDeath, csSetting.monsterDeathAnimation);
		GetComponent<Animation>()[csSetting.monsterDeathAnimation].wrapMode = WrapMode.Once;
		GetComponent<Animation>().playAutomatically = false;		
		GetComponent<Animation>().Play (csSetting.monsterDeathAnimation);
